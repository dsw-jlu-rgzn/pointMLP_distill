...
def main():
    ...
    optimizer = torch.optim.SGD(net_s.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)
    if optimizer_dict is not None:
        optimizer.load_state_dict(optimizer_dict)
    scheduler = CosineAnnealingLR(optimizer, args.epoch, eta_min=args.learning_rate / 100, last_epoch=start_epoch - 1)
    net_ts = {'net_t':net_t, 'net_s':net_s}
    criterion_all = {'cls_criterion':criterion, 'kd_criterion':criterionKD}
    for epoch in range(start_epoch, args.epoch):
        printf('Epoch(%d/%s) Learning Rate %s:' % (epoch + 1, args.epoch, optimizer.param_groups[0]['lr']))
        train_out = train(net_ts, train_loader, optimizer, criterion_all, device, args)  # {"loss", "acc", "acc_avg", "time"}
        test_out = validate(net_ts, test_loader, criterion_all, device)
        scheduler.step()
